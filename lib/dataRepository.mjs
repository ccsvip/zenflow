...
export async function listRequirements() {
  const result = await query(
    `SELECT id, title, description, stakeholder, project_id AS "projectId", priority, status, attachments
     FROM requirements
     ORDER BY created_at ASC`,
  );
  return result.rows;
...
export async function createRequirement(data) {
  ...
  const result = await query(
    `INSERT INTO requirements (id, title, description, stakeholder, project_id, priority, status, attachments)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING id, title, description, stakeholder, project_id AS "projectId", priority, status, attachments`,
    [
      id,
      data.title,
...
export async function updateRequirement(id, data) {
  const result = await query(
    `UPDATE requirements
     SET title = $2, description = $3, stakeholder = $4, project_id = $5, priority = $6, status = $7, attachments = $8
     WHERE id = $1
     RETURNING id, title, description, stakeholder, project_id AS "projectId", priority, status, attachments`,
    [
      id,
